...
ImGuiStyle::ImGuiStyle()
{
	...
	WindowRounding = 0.f;    //roundingbok 7         // Radius of window corners rounding. Set to 0.0f to have rectangular windows
	...
	ChildBorderSize = 1.0f;  //1            // Thickness of border around child windows. Generally set to 0.0f or 1.0f. Other values not well tested.
...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			//g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(178, 0, 62, 255))
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(148, 0, 217, 255)); // White fill
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	auto color_grayblue = GetColorU32(ImVec4(0.58f, 0.00f, 0.85f, 1.f));
	//auto color_grayblue = GetColorU32(ImVec4(1.00f, 0.00f, 0.23f, 1.f));
	auto color_deepblue = GetColorU32(ImVec4(0.68f, 0.f, 0.85f, 0.40f));
	//auto color_deepblue = GetColorU32(ImVec4(1.00f, 0.f, 0.23f, 0.40f)); gradienttextshit
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(2, 2), grab_bb.Max - ImVec2(0, -1), color_grayblue, color_grayblue, color_deepblue, color_deepblue);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (selected)
	{
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.58f, 0.00f, 0.85f, 1.f)); //listboxcolor
