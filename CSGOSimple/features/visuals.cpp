...
{
	esp_ctx.pl = pl;
	esp_ctx.dangerzone = g_Options.dangerzone;
...
void Visuals::Player::HeadEsp(C_BasePlayer* entity)
{
	...
	{
		g_VGuiSurface->DrawSetColor(g_Options.HeadEspColor);
		Draw3dCube(5.f, eyepos, cross, g_Options.HeadEspColor);
	}
...
{
	int w1, h1;
	g_EngineClient->GetScreenSize(w1, h1);
	...
	float fps = 1.f / g_GlobalVars->absoluteframetime;
	char *FPS;
	...
	const wchar_t* watermark = L"hellcat | beta";
	...
	DrawString(watermark_font, 10, 50, Color(200, 200, 200, 255), FONT_LEFT, "Local Time: %s", GetTimeString().c_str());
	//DrawString(watermark_font, 10, 60, Color::FromHSB(rainbow, 1.f, 1.f), FONT_LEFT, "Ping: %s", PING);
	DrawString(watermark_font, 10, 40, Color(200, 200, 200, 255), FONT_LEFT, "User: %s", computer);
	DrawString(watermark_font, 10, 30, Color::FromHSB(rainbow, 1.f, 1.f), FONT_LEFT, "FPS: %s", FPS);
...
void Visuals::RenderItemEsp(C_BaseEntity* ent)
{
	wchar_t buf[80];
	Vector points_transformed[8];
	std::string itemstr = "Cash";
	std::string itemstrfinal = "Cash";
	...
	if (MultiByteToWideChar(CP_UTF8, 0, itemstrfinal.c_str(), -1, buf, 80) > 0) {
		int w = bbox.right - bbox.left;
		int tw, th;
		g_VGuiSurface->GetTextSize(esp_font, buf, tw, th);
...
//--------------------------------------------------------------------------------
void Visuals::Misc::RenderCrosshair(C_BaseCombatWeapon *weapon)
{
	int w, h;
	...
	if (weapon->IsSniper())
	{
		g_VGuiSurface->DrawLine(cx - 35, cy, cx + 35, cy);
		g_VGuiSurface->DrawLine(cx, cy - 35, cx, cy + 35);
	}
}
...
void Visuals::NightMode()
{
	...
	auto night_val = g_Options.nightmode_intensity;
