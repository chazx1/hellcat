	...
	matrix3x4_t *matrix)
{
	...
	bool is_weapon = strstr(mdl->szName, "models/weapons/v") || strstr(mdl->szName, "_dropped.mdl") != nullptr;


	auto materials = g_MatSystem->FindMaterial(mdl->szName, TEXTURE_GROUP_MODEL);
	auto worldxd = g_MatSystem->FindMaterial(mdl->szName, TEXTURE_GROUP_WORLD);
	...
	if (is_weapon && !is_arm && g_Options.chams_weapon_enabled) {
		auto material = g_MatSystem->FindMaterial(mdl->szName, TEXTURE_GROUP_MODEL);
		g_MdlRender->ForcedMaterialOverride(material);


		if (g_Options.chams_weapon_ignorez) {
			OverrideMaterial(
				true,
				...
				g_Options.color_chams_weapon_occluded);
			...
			OverrideMaterial(
				...
				g_Options.chams_weapon_wireframe,
				false,
				g_Options.color_chams_weapon_visible);
		}
		else {
			OverrideMaterial(
				false,
