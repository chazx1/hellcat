...
//TimeWarp* CreateMove = new CBacktrackHelper;
vgui::HFont backtrackfont;
void TimeWarp::CreateMove(CUserCmd* cmd)
{
	backtrackfont = g_VGuiSurface->CreateFont_();
	g_VGuiSurface->SetFontGlyphSet(backtrackfont, "Tahoma", 13, 350, 0, 0, FONTFLAG_OUTLINE, FONTFLAG_ANTIALIAS);
								...
								for (int t = 0; t < 13; ++t)
								{
									...
									if (TimeWarpData[i][t].simtime && TimeWarpData[i][t].simtime + 1 > pLocal->m_flSimulationTime())
									{
										if (Math::WorldToScreen(TimeWarpData[i][t].hitboxPos, screenbacktrack[i][t]))
										{
											...
											g_VGuiSurface->DrawSetTextFont(backtrackfont);
											wchar_t buf[128];
